...
{ 
    ...
    {     
        ...
        public static void GridOnDeath(Pawn_HealthTracker __instance)
        {
            ...
            if (pawn != null && pawn.Spawned && pawn.Faction != null && pawn.Faction.HostileTo(Faction.OfPlayer) && pawn.Faction != Faction.OfInsects)
            {
                var avoidGrid = pawn.Map.avoidGrid;
                if (avoidGrid?.Grid != null)
                {
                    int cellIndex = pawn.Map.cellIndices.CellToIndex(pawn.Position);
                    var clearPaths = avoidGrid.Grid[cellIndex] == 0;
